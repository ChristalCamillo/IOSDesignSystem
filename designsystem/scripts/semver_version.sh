...
get_incremented_semantic_version () {
  ...
  echo "${version_prefix}$semantic_version$version_suffix"
}

get_semantic_version_from_git () {
#  local latest_tag=$(git rev-list --tags --max-count=1)
  local filterGrep="^${version_prefix_tag}[0-9]*.[0-9]*.[0-9]*${version_suffix}[0-9]*$"

  local latest_tag=$(git ls-remote --tags --sort=v:refname | sed 's/.*\///; s/\^{}//' | grep "$filterGrep" | tail -n1 | sed "s/${version_prefix_tag}//")
...
while getopts ":i:o:t:p:f:l:m:b:g:s:" opt; do
  case ${opt} in
    i )
